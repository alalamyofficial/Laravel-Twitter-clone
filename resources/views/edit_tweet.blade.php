...
<form action="{{route('tweet.update',$tweet->id)}}" method="POST" enctype="multipart/form-data">
    ...
    <textarea name="body"  id="example2" data-emoji-input="unicode" 
        data-emojiable="true" class="w-full" placeholder="What's up doc ?">
        {{$tweet->body}}
    </textarea>
    ...
    <footer class="flex justify-between items-center">
        ...
        <img id="old_image" width="50" height="50" src="{{asset($tweet->image)}}"/> 

        <i class="fas fa-arrow-alt-circle-right fa-2x"></i>

        <img id="blah2" width="50" height="50" />


        <button
                type="submit"
                class="bg-green-400 hover:bg-green-600 rounded-lg shadow px-10 text-sm text-white h-10"
            >
                Update
        </button>
